...
async function bootstrap(): Promise<void> {
  const logger = createAppLogger({ service: 'assignment-service' });
  ...
  if (process.env.NODE_ENV !== 'production') {
    const swaggerConfig = new DocumentBuilder()
      ...
      .setDescription('Assignments, grading, submissions')
      .setVersion('1.0')
  ...
  logger.info(`Assignment service listening on port ${port}`);
}

bootstrap().catch((err) => {
  console.error('Assignment service failed to start', err);
  process.exit(1);
});
